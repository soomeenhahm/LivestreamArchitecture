...
public class WorldAgentDay5WS : MonoBehaviour
{
    public float worldBounds;
    public int growthGens;
    public string commandText;[HideInInspector]
    public string userText;[HideInInspector]
    public string nnText;[HideInInspector]

    private Vector3 dlaTarget;

    public Transform Target; // transform that the camera will follow
    ...
    private List<GameObject> allGeo;
    ...
    void Start()
    {
        ...
        catogorizeObjs();
        allGeo = new List<GameObject>();
    }

    public void Refresh()
    {
        foreach (GameObject go in allGeo)
        {
            Destroy(go);
            commandText = " ";
        }
    ...
    private void Update()
    {
        if (curObj != null)
        {
            Target.position = Vector3.Lerp(Target.position, curObj.transform.position, Time.deltaTime * delay); // lerp between target position and curObj position
        }
        else
        {
    ...
    public void stringSort(string serverText)//string inString
    {
        ...
        for (int i = 0; i < growthGens; i++)
        {
            ...
            {
                dlaTarget = new Vector3(Random.Range(-worldBounds , worldBounds ), Random.Range(worldBounds / 2, worldBounds), Random.Range(-worldBounds , worldBounds));
            }
            ...
            {
                dlaTarget = new Vector3(Random.Range(0, worldBounds), Random.Range(0, worldBounds), Random.Range(-worldBounds / 2, worldBounds / 2));
            }
            else if (commandText.Contains("left"))
            {
                dlaTarget = new Vector3(Random.Range(-worldBounds, 0), Random.Range(0, worldBounds), Random.Range(-worldBounds / 2, worldBounds / 2));
            }
            else if (commandText.Contains("forward"))
            {
                dlaTarget = new Vector3(Random.Range(-worldBounds / 2, worldBounds / 2), Random.Range(0, worldBounds), Random.Range(0, worldBounds));
            }
            else if (commandText.Contains("back"))
            {
                dlaTarget = new Vector3(Random.Range(-worldBounds / 2, worldBounds / 2), Random.Range(0, worldBounds), -Random.Range(-worldBounds, 0));
            }
            else
            {
    ...
    private void OnGUI()
    {
        ...
        GUI.Label(new Rect(20, 35, 200, 50), "command: ");
        GUI.Label(new Rect(20, 50, 200, 50), "by: ");
        
        GUI.contentColor = new Color(0.1f, 0.35f, 0.40f);
        GUI.Label(new Rect(100, 35, 200, 50), commandText);
        GUI.Label(new Rect(50, 50, 200, 50), userText);

        GUI.skin.label.fontSize = 11;
        int s = 75;
        int v = 13;
        GUI.contentColor = Color.black;
        GUI.Label(new Rect(20, s, 200, 50), "Anger: ");
        GUI.Label(new Rect(20, s = s + v, 200, 50), "Anticipation: ");
        GUI.Label(new Rect(20, s = s + v, 200, 50), "Disgust: ");
        GUI.Label(new Rect(20, s = s + v, 200, 50), "Fear: ");
        GUI.Label(new Rect(20, s = s + v, 200, 50), "Joy: ");
        GUI.Label(new Rect(20, s = s + v, 200, 50), "Sadness: ");
        GUI.Label(new Rect(20, s = s + v, 200, 50), "Surprise: ");
        GUI.Label(new Rect(20, s = s + v + v, 200, 50), "Count: ");

        int n = 90;
        if (nnVals.Count == 7)
        {
            GUI.contentColor = new Color(0.1f, 0.35f, 0.40f);
            GUI.Label(new Rect(n, s = 75, 200, 50), $"{nnVals[0] * 100:0.00}" + "%");
            GUI.Label(new Rect(n, s = s + v, 200, 50), $"{nnVals[1] * 100:0.00}" + "%");
            GUI.Label(new Rect(n, s = s + v, 200, 50), $"{nnVals[2] * 100:0.00}" + "%");
            GUI.Label(new Rect(n, s = s + v, 200, 50), $"{nnVals[3] * 100:0.00}" + "%");
            GUI.Label(new Rect(n, s = s + v, 200, 50), $"{nnVals[4] * 100:0.00}" + "%");
            GUI.Label(new Rect(n, s = s + v, 200, 50), $"{nnVals[5] * 100:0.00}" + "%");
            GUI.Label(new Rect(n, s = s + v, 200, 50), $"{nnVals[6] * 100:0.00}" + "%");

            GUI.Label(new Rect(n-30, s = s + v + v, 200, 50), $"{transform.childCount}");
        }
